...
import de.mhus.osgi.dev.cache.ehcache.jsr107.config.Jsr107CacheConfiguration;
import de.mhus.osgi.dev.cache.ehcache.jsr107.internal.Jsr107CacheLoaderWriter;
...
import static de.mhus.osgi.dev.cache.ehcache.jsr107.CloseUtil.closeAllAfter;
...
public class ConfigurationMerger {
        ...
        try {
            ...
            if (xmlConfiguration != null && templateName != null) {
                ...
                try {
                    templateBuilder =
                            xmlConfiguration.newCacheConfigurationBuilderFromTemplate(
                                    templateName,
                                    jsr107Configuration.getKeyType(),
                                    jsr107Configuration.getValueType());
                ...
                if (templateBuilder != null) {
                    ...
                    LOG.info(
                            "Configuration of cache {} will be supplemented by template {}",
                            cacheName,
                            templateName);
                }
            }
            ...
            if (hasConfiguredExpiry) {
                LOG.info(
                        "Cache {} will use expiry configuration from template {}",
                        cacheName,
                        templateName);
            } else {
            ...
            CacheLoaderWriterConfiguration<?> ehcacheLoaderWriterConfiguration =
                    builder.getService(DefaultCacheLoaderWriterConfiguration.class);
            if (ehcacheLoaderWriterConfiguration == null) {
                ...
                if (loaderWriter != null
                        && (jsr107Configuration.isReadThrough()
                                || jsr107Configuration.isWriteThrough())) {
                ...
                if (!jsr107Configuration.isReadThrough() && !jsr107Configuration.isWriteThrough()) {
                    LOG.warn(
                            "Activating Ehcache loader/writer for JSR-107 cache {} which was neither read-through nor write-through",
                            cacheName);
                }
                LOG.info(
                        "Cache {} will use loader/writer configuration from template {}",
                        cacheName,
                        templateName);
            }
            ...
            setupManagementAndStatsInternal(
                    jsr107Configuration,
                    findSingletonAmongst(
                            Jsr107CacheConfiguration.class,
            ...
            return new ConfigHolder<>(
                    new CacheResources<>(
                            ...
                            loaderWriter,
                            expiryPolicy,
                            initCacheEventListeners(jsr107Configuration)),
                    new Eh107CompleteConfiguration<>(
                            jsr107Configuration,
                            cacheConfiguration,
                            hasConfiguredExpiry,
            ...
            if (throwable instanceof IllegalArgumentException) {
                throw closeAllAfter(
                        (IllegalArgumentException) throwable, expiryPolicy, loaderWriter);
            } else {
    ...
    private <K, V> CacheConfigurationBuilder<K, V> handleStoreByValue(
            Eh107CompleteConfiguration<K, V> jsr107Configuration,
            CacheConfigurationBuilder<K, V> builder,
            String cacheName) {
        @SuppressWarnings("unchecked")
        Collection<DefaultCopierConfiguration<?>> copierConfigs =
                builder.getServices(
                        (Class<DefaultCopierConfiguration<?>>)
                                (Class) DefaultCopierConfiguration.class);
        if (copierConfigs.isEmpty()) {
            if (jsr107Configuration.isStoreByValue()) {
                if (xmlConfiguration != null) {
                    ...
                    if (defaultCopyProviderConfiguration != null) {
                        Map<Class<?>, DefaultCopierConfiguration<?>> defaults =
                                defaultCopyProviderConfiguration.getDefaults();
                            ...
                            builder =
                                    builder.withService(
                                            new DefaultCopierConfiguration<>(
                                                    SerializingCopier.<K>asCopierClass(),
                        ...
                        if (matchingDefault) {
                            LOG.info(
                                    "CacheManager level copier configuration overwriting JSR-107 by-value semantics for cache {}",
                                    cacheName);
                        }
                        return builder;
                    }
            ...
            LOG.info(
                    "Cache level copier configuration overwriting JSR-107 by-value semantics for cache {}",
                    cacheName);
        }
        return builder;
    }
    ...
    private static <K, V> CacheConfigurationBuilder<K, V> addDefaultCopiers(
            CacheConfigurationBuilder<K, V> builder, Class<K> keyType, Class<V> valueType) {
        ...
        if (immutableTypes.contains(keyType)) {
            builder =
                    builder.withService(
                            new DefaultCopierConfiguration<K>(
                                    (Class) Eh107IdentityCopier.class,
                                    DefaultCopierConfiguration.Type.KEY));
        } else {
            builder =
                    builder.withService(
    ...
    private static void handleCopierDefaultsforImmutableTypes(
            Map<Class<?>, DefaultCopierConfiguration<?>> defaults) {
    ...
    private static void addIdentityCopierIfNoneRegistered(
            Map<Class<?>, DefaultCopierConfiguration<?>> defaults, Class<?> clazz) {
        if (!defaults.containsKey(clazz)) {
            defaults.put(
                    clazz,
                    new DefaultCopierConfiguration(
                            Eh107IdentityCopier.class, DefaultCopierConfiguration.Type.VALUE));
        }
    }

    private <K, V>
            Map<CacheEntryListenerConfiguration<K, V>, ListenerResources<K, V>>
                    initCacheEventListeners(CompleteConfiguration<K, V> config) {
        Map<CacheEntryListenerConfiguration<K, V>, ListenerResources<K, V>> listenerResources =
                new ConcurrentHashMap<>();
        for (CacheEntryListenerConfiguration<K, V> listenerConfig :
                config.getCacheEntryListenerConfigurations()) {
            listenerResources.put(
                    listenerConfig, ListenerResources.createListenerResources(listenerConfig));
        }
    ...
    private <K, V> Jsr107CacheLoaderWriter<K, V> initCacheLoaderWriter(
        ...
        Factory<CacheWriter<K, V>> cacheWriterFactory =
                (Factory<CacheWriter<K, V>>) (Object) config.getCacheWriterFactory();

        if (config.isReadThrough() && cacheLoaderFactory == null) {
            throw new IllegalArgumentException(
                    "read-through enabled without a CacheLoader factory provided");
        }
        if (config.isWriteThrough() && cacheWriterFactory == null) {
            throw new IllegalArgumentException(
                    "write-through enabled without a CacheWriter factory provided");
        }
        ...
        if (cacheLoader == null && cacheWriter == null) {
            ...
            return new Eh107CacheLoaderWriter<>(
                    cacheLoader, config.isReadThrough(), cacheWriter, config.isWriteThrough());
        }
    }

    void setUpManagementAndStats(
            InternalCache<?, ?> cache, Eh107Configuration<?, ?> configuration) {
    ...
    private void setupManagementAndStatsInternal(
            Eh107Configuration<?, ?> configuration, Jsr107CacheConfiguration cacheConfiguration) {
        ...
        if (cacheConfiguration != null) {
            ...
            if (managementEnabled != null
                    && managementEnabled != ConfigurationElementState.UNSPECIFIED) {
            ...
            if (statisticsEnabled != null
                    && statisticsEnabled != ConfigurationElementState.UNSPECIFIED) {
