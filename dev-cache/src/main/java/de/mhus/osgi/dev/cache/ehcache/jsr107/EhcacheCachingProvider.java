...
public class EhcacheCachingProvider implements CachingProvider {
    ...
    /**
     * Enables to create a JSR-107 {@link CacheManager} based on the provided Ehcache {@link
     * Configuration}.
     *
    ...
    /**
     * Enables to create a JSR-107 {@link CacheManager} based on the provided Ehcache {@link
     * Configuration} with the provided {@link Properties}.
     *
     * @param uri the URI identifying this cache manager
     * @param config the Ehcache configuration to use
    ...
    Eh107CacheManager getCacheManager(ConfigSupplier configSupplier, Properties properties) {
        ...
        synchronized (cacheManagers) {
            ...
            if (cacheManager == null || cacheManager.isClosed()) {
                ...
                cacheManager =
                        createCacheManager(uri, configSupplier.getConfiguration(), properties);
    ...
    public void close() {
        synchronized (cacheManagers) {
            for (Map.Entry<ClassLoader, ConcurrentMap<URI, Eh107CacheManager>> entry :
    ...
    void close(Eh107CacheManager cacheManager) {
        synchronized (cacheManagers) {
            final ConcurrentMap<URI, Eh107CacheManager> map =
                    cacheManagers.get(cacheManager.getClassLoader());
