...
public class CmdDevHazelcast extends AbstractCmd {
    ...
    public Object execute2() throws Exception {
        ...
        if (cmd.equals("clustermanager")) {
            ...
            for (Entry<String, CacheSimpleConfig> cc :
                    inst.getConfig().getCacheConfigs().entrySet()) {
        ...
        if (cmd.equals("lock")) {
            String lockName =
                    parameters == null || parameters.length == 0 ? "testlock" : parameters[0];
            HazelcastClusterManager hccm = (HazelcastClusterManager) clusterManager;
            HazelcastInstance inst = hccm.getInstance();
    ...
    //    protected <K, V, C extends Configuration<K, V>> CacheConfig<K, V> createCacheConfig(String
    // cacheName,
    //            C configuration) {
