...
package de.mhus.osgi.dev.cache.ehcache.jsr107.internal;
...
public class Jsr107ServiceConfigurationParser
    ...
    private static final URL XML_SCHEMA =
            Jsr107ServiceConfigurationParser.class.getResource("/ehcache-107-ext.xsd");
    ...
    @Override
    public ServiceCreationConfiguration<Jsr107Service, ?> parseServiceCreationConfiguration(
            final Element fragment, ClassLoader classLoader) {
        ...
        if (fragment.hasAttribute(JSR_107_COMPLIANT_ATOMICS_ATTRIBUTE)) {
            jsr107CompliantAtomics =
                    parseBoolean(fragment.getAttribute(JSR_107_COMPLIANT_ATOMICS_ATTRIBUTE));
        }
        if (fragment.hasAttribute(ENABLE_MANAGEMENT_ALL_ATTRIBUTE)) {
            enableManagementAll =
                    parseBoolean(fragment.getAttribute(ENABLE_MANAGEMENT_ALL_ATTRIBUTE))
        ...
        if (fragment.hasAttribute(ENABLE_STATISTICS_ALL_ATTRIBUTE)) {
            enableStatisticsAll =
                    parseBoolean(fragment.getAttribute(ENABLE_STATISTICS_ALL_ATTRIBUTE))
                            ? ConfigurationElementState.ENABLED
                            : ConfigurationElementState.DISABLED;
        }
        ...
        for (int i = 0; i < childNodes.getLength(); i++) {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                templates.put(
                        item.getAttribute(CACHE_NAME_ATTRIBUTE),
                        item.getAttribute(TEMPLATE_NAME_ATTRIBUTE));
            }
        }

        return new Jsr107Configuration(
                defaultTemplate,
                templates,
                jsr107CompliantAtomics,
                enableManagementAll,
    ...
    @Override
    public Element unparseServiceCreationConfiguration(
            ServiceCreationConfiguration<Jsr107Service, ?> serviceCreationConfiguration) {
        throw new XmlConfigurationException(
                "XML translation of JSR-107 cache elements are not supported");
    }
}
