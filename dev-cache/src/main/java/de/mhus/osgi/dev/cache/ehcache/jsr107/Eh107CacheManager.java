...
import de.mhus.osgi.dev.cache.ehcache.jsr107.internal.WrappedCacheLoaderWriter;

import static de.mhus.osgi.dev.cache.ehcache.jsr107.CloseUtil.chain;
import static de.mhus.osgi.dev.cache.ehcache.jsr107.CloseUtil.closeAll;
...
public class Eh107CacheManager implements CacheManager {
    ...
    private void refreshAllCaches() {
        for (Map.Entry<String, CacheConfiguration<?, ?>> entry :
                ehCacheManager.getRuntimeConfiguration().getCacheConfigurations().entrySet()) {
        ...
        for (Map.Entry<String, Eh107Cache<?, ?>> namedCacheEntry : caches.entrySet()) {
            ...
            if (!cache.isClosed()) {
                ...
                Eh107Configuration<?, ?> configuration =
    ...
    private <K, V> Eh107Cache<K, V> wrapEhcacheCache(
            String alias, CacheConfiguration<K, V> ehConfig) {
        org.ehcache.Cache<K, V> cache =
                ehCacheManager.getCache(alias, ehConfig.getKeyType(), ehConfig.getValueType());
    ...
    private <K, V> Eh107Cache<K, V> wrapEhcacheCache(String alias, InternalCache<K, V> cache) {
        ...
        Eh107Configuration<K, V> config =
                new Eh107ReverseConfiguration<>(
                        cache,
                        cacheLoaderWriter != null,
                        cacheLoaderWriter != null,
                        storeByValueOnHeap);
        configurationMerger.setUpManagementAndStats(cache, config);
        Eh107Expiry<K, V> expiry =
                new EhcacheExpiryWrapper<>(cache.getRuntimeConfiguration().getExpiryPolicy());
        CacheResources<K, V> resources =
                new CacheResources<>(alias, wrapCacheLoaderWriter(cacheLoaderWriter), expiry);
        return new Eh107Cache<>(alias, config, resources, cache, statisticsService, this);
    }

    private <K, V> Jsr107CacheLoaderWriter<K, V> wrapCacheLoaderWriter(
    ...
    public <K, V, C extends Configuration<K, V>> Cache<K, V> createCache(String cacheName, C config)
        ...
        synchronized (cachesLock) {
            if (config instanceof Eh107Configuration.Eh107ConfigurationWrapper) {
                @SuppressWarnings("unchecked")
                Eh107Configuration.Eh107ConfigurationWrapper<K, V> configurationWrapper =
                        (Eh107Configuration.Eh107ConfigurationWrapper<K, V>) config;
            ...
            ConfigurationMerger.ConfigHolder<K, V> configHolder =
                    configurationMerger.mergeConfigurations(cacheName, config);

            final InternalCache<K, V> ehCache;
            try {
                ehCache =
                        (InternalCache<K, V>)
                                ehCacheManager.createCache(
                                        cacheName, configHolder.cacheConfiguration);
            } catch (IllegalArgumentException e) {
                throw configHolder.cacheResources.closeResourcesAfter(
                        new CacheException("A Cache named [" + cacheName + "] already exists"));
            } catch (Throwable t) {
            ...
            try {
                if (configHolder.useEhcacheLoaderWriter) {
                    cacheResources =
                            new CacheResources<>(
                                    cacheName,
                                    wrapCacheLoaderWriter(ehCache.getCacheLoaderWriter()),
                                    cacheResources.getExpiryPolicy(),
                                    cacheResources.getListenerResources());
                }
                cache =
                        new Eh107Cache<>(
                                cacheName,
                                new Eh107CompleteConfiguration<>(
                                        configHolder.jsr107Configuration,
    ...
    public <K, V> Cache<K, V> getCache(String cacheName, Class<K> keyType, Class<V> valueType) {
        ...
        if (keyType != actualKeyType) {
            throw new ClassCastException(
                    "Cache has key type "
                            + actualKeyType.getName()
                            + ", but getCache() called with key type "
                            + keyType.getName());
        }

        if (valueType != actualValueType) {
            throw new ClassCastException(
                    "Cache has value type "
                            + actualValueType.getName()
                            + ", but getCache() called with value type "
                            + valueType.getName());
        }
                ...
                chain(
                        ...
                        () -> {
                            if (ehCacheManager instanceof PersistentCacheManager) {
                                try {
                                    ((PersistentCacheManager) ehCacheManager)
                                            .destroyCache(cache.getName());
