...
package de.mhus.osgi.dev.cache.ehcache.jsr107.internal.classes;
...
abstract class MemberUtils {
     ...
     * some JVMs, using reflection to invoke these members seems to (wrongly) prevent access even
     * when the modifier is {@code public}. Calling {@code setAccessible(true)} solves the problem
     * but will only work from sufficiently privileged code. Better workarounds would be gratefully
     * accepted.
     *
     ...
     * runtime parameter types, such that a list ordered by the results of the comparison would
    ...
    private static float getTotalTransformationCost(
        ...
        if (isVarArgs) {
            ...
            if (noVarArgsPassed) {
                // When no varargs passed, the best match is the most generic matching type, not the
                // most specific.
    ...
    /**
     * A class providing a subset of the API of java.lang.reflect.Executable in Java 1.8, providing
     * a common representation for function signatures for Constructors and Methods.
     */
